...
file_path = os.path.join(base_dir, 'Backlog_13.xlsx')
...
pdf_path = os.path.join(diretorio_relatorio, "Graficos_Backlog_ITI_Semana_13_2026.pdf")
...
fig.suptitle('Análise Backlog ITI - Semana 06/04 a 10/04', fontsize=16)

# Função auxiliar para adicionar rótulos de dados
def to_native(val):
    """Converte valores numpy para nativos do Python."""
...
def add_labels_bars(ax):
    ...
    for p in ax.patches:
        h = to_native(p.get_height())
        x = to_native(p.get_x())
        w = to_native(p.get_width())
        ax.annotate(f'{h}', (x + w / 2, h), ha='center', va='bottom')

def add_labels_line(ax, x_data, y_data):
    """Adiciona rótulos de dados nas linhas do gráfico."""
    for i, (x, y) in enumerate(zip(x_data, y_data)):
        y_val = to_native(y)
        # Use integer index for categorical x-axis to avoid unit conversion issues
        x_val = float(i)
        
        ax.annotate(f'{y_val}', (x_val, float(y_val)), textcoords="offset points", xytext=(0,5), ha='center')
...
for i, total in enumerate(df_total_sector.values):
    total = to_native(total)
    # Para a barra 'Total'
    axs[0, 0].text(float(i), float(total) / 2, str(total), ha='center', va='center', color='black')  # Total no meio
    if total > 0:
        axs[0, 0].text(float(i), (float(total) / 2) - 1, f'({(total / total * 100):.1f}%)',  # Percentual logo abaixo do Total
                   ha='center', va='top', color='black')  

for i, resolved in enumerate(df_resolved_sector.reindex(df_total_sector.index, fill_value=0).values):
    resolved = to_native(resolved)
    # Para as barras 'Resolvidos'
    axs[0, 0].text(float(i + bar_width), float(resolved) / 2, str(resolved), ha='center', va='center', color='black')  # Resolvidos no meio
    val_total = to_native(df_total_sector.iloc[i])
    pct = (resolved / val_total * 100) if val_total > 0 else 0
    axs[0, 0].text(float(i + bar_width), (float(resolved) / 2) - 1, f'({pct:.1f}%)', 
...
for i, unresolved in enumerate(df_unresolved_sector.values):
    unresolved = to_native(unresolved)
    # Para as barras 'Pendentes'
    axs[0, 0].text(float(i + 2 * bar_width), float(unresolved) / 2, str(unresolved), ha='center', va='center', color='black')  # Pendentes no meio
    val_total = to_native(df_total_sector.iloc[i])
    pct = (unresolved / val_total * 100) if val_total > 0 else 0
    axs[0, 0].text(float(i + 2 * bar_width), (float(unresolved) / 2) - 1, f'({pct:.1f}%)', 
...
if len(df_responsavel) > 5:
    ...
    responsaveis_principais = df_responsavel.head(5).copy()
...
for i, (bar_resolved, bar_pending) in enumerate(zip(resolved_bars, pending_bars)):
    resolved_height = to_native(bar_resolved.get_height())
    pending_height = to_native(bar_pending.get_height())
    total_incidents = to_native(responsaveis_principais.values[i])
    ...
    if pending_height > 0:
        ...
        y_pos_pending = resolved_height + (pending_height / 2)
        axs[1, 1].text(
